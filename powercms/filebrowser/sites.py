...
from django.core import files as FILES
...
from powercms.filebrowser.base import FileListing, FileObject
from powercms.filebrowser.decorators import path_exists, file_exists
from powercms.filebrowser.models import LogEntry
from powercms.filebrowser.storage import FileSystemStorageMixin
from powercms.filebrowser.templatetags.fb_tags import query_helper
...
class FileBrowserSite(object):
    ...
    def get_urls(self):
        ...
        urlpatterns = [
            ...
            url(r'^browse/$', csrf_exempt(path_exists(self, filebrowser_view(self.browse))), name="fb_browse"),
            url(r'^createdir/', csrf_exempt(path_exists(self, filebrowser_view(self.createdir))), name="fb_createdir"),
            url(r'^upload/', csrf_exempt(path_exists(self, filebrowser_view(self.upload))), name="fb_upload"),
            url(r'^delete_confirm/$', csrf_exempt(file_exists(self, path_exists(self, filebrowser_view(self.delete_confirm)))), name="fb_delete_confirm"),
            url(r'^delete/$', csrf_exempt(file_exists(self, path_exists(self, filebrowser_view(self.delete)))), name="fb_delete"),
            url(r'^detail/$', csrf_exempt(file_exists(self, path_exists(self, filebrowser_view(self.detail)))), name="fb_detail"),
            url(r'^history/$', csrf_exempt(file_exists(self, path_exists(self, filebrowser_view(self.history)))), name="fb_history"),
            url(r'^version/$', csrf_exempt(file_exists(self, path_exists(self, filebrowser_view(self.version)))), name="fb_version"),
    ...
    def create_log(self, request, object_id, action_flag, change_message=''):
        LogEntry(
            user=request.user,
            object_id=object_id,
    ...
    def check_dir(self, path, create=False):
    ...
    def browse(self, request):
        ...
        for fileobject in listing:
            ...
            if fileobject.filetype == "Image":
                # path_thumbnail
                root = os.path.join(MEDIA_ROOT, 'thumbnails')
                self.check_dir(root, create=True)
                filename = os.path.basename(os.path.join(MEDIA_ROOT, fileobject.path))
                name_clean, extension = os.path.splitext(filename)
                filename = name_clean+'_thumbnails' + extension
                path_thumbnails = os.path.join(root, filename)
                if not os.path.exists(path_thumbnails):
                    try:
                        image = Image.open(os.path.join(MEDIA_ROOT,fileobject.path))
                        MAX_SIZE = (100, 100)
                        image.thumbnail(MAX_SIZE)
                        image.save(path_thumbnails)
                    except FileNotFoundError: # caso o storage seja django-storage[google]
                        response = requests.get(fileobject.url)
                        ...
                        for block in response.iter_content(1024 * 8):
                            ...
                            lf.write(block)
                        image = Image.open(FILES.File(lf))
                        MAX_SIZE = (100, 100)
                        image.thumbnail(MAX_SIZE)
                        image.save(path_thumbnails)

                setattr(fileobject, 'thumbnails', '/media/thumbnails/%s' % filename)
        ...
        return render(request, 'filebrowser/index.html', {
            ...
            'filelisting': filelisting,
            'query': query,
            'title': _(u'Gerenciador de Arquivos'),
    ...
    def createdir(self, request):
        "Create Directory"
        from powercms.filebrowser.forms import CreateDirForm
        query = request.GET
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    self.create_log(request, server_path, 1)
    ...
    def delete(self, request):
        ...
        if request.GET:
            try:
                ...
                fileobject.delete()
                file_thumb, extension = os.path.splitext(fileobject.filename)
                file_thumb += '_thumbnails' + extension
                if self.check_dir(os.path.join(MEDIA_ROOT, 'thumbnails', file_thumb)):
                    os.remove(os.path.join(MEDIA_ROOT, 'thumbnails', file_thumb))
                ...
                self.create_log(request, fileobject.path, 3)
            except OSError:
    ...
    def detail(self, request):
        ...
        """
        from powercms.filebrowser.forms import ChangeForm
        query = request.GET
        path = u'%s' % os.path.join(self.directory, query.get('dir', ''))
        fileobject = FileObject(os.path.join(path, query.get('filename', '')), site=self)
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    if new_name != fileobject.filename:
                        old_name = fileobject.filename
                        old_path = fileobject.path
                        ...
                        self.create_log(request, os.path.join(fileobject.head, new_name), 2, u'Arquivo renomeado de %s para %s' % (old_name, new_name))
                        LogEntry.objects.filter(object_id=old_path).update(object_id=os.path.join(fileobject.head, new_name))
                        return HttpResponseRedirect('%s?dir=%s&filename=%s' % (reverse("filebrowser:fb_detail", current_app=self.name), query.get('dir', ''), new_name))
                    ...
                    if new_file != fileobject.read:
                        ...
                        self.create_log(request, fileobject.path, 2, u'Arquivo editado')
    ...
    def history(self, request):
        ...
        action_list = LogEntry.objects.filter(object_id=fileobject.path)

        return render(request, 'filebrowser/history.html', {
            'fileobject': fileobject,
            'action_list': action_list,
            'query': query,
            'title': u'Histórico: %s' % fileobject.filename,
            'is_popup': "pop" in request.GET,
            'settings_var': get_settings_var(directory=self.directory),
            'breadcrumbs': get_breadcrumbs(query, query.get('dir', '')),
            'breadcrumbs_title': u'%s &rsaquo; Histórico' % fileobject.filename,
            'filebrowser_site': self,
            'current_app': self.name
        })
    ...
    def version(self, request):
        ...
        return render('filebrowser/version.html',{
            'fileobject': fileobject,
            'query': query,
            'settings_var': get_settings_var(directory=self.directory),
            'filebrowser_site': self,
    ...
    def _upload_file(self, request):
        ...
        if request.method == "POST":
            ...
            if file_already_exists and OVERWRITE_EXISTING:
                ...
                self.create_log(request, old_file, 2, u'Arquivo substituído.')
            else:
                file_name = smart_text(uploadedfile)
                filedata.name = os.path.relpath(file_name, path)
                self.create_log(request, file_name, 1)
...
# Default actions
from powercms.filebrowser.actions import flip_horizontal, flip_vertical, rotate_90_clockwise, rotate_90_counterclockwise, rotate_180, move
